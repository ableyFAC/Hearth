...
-- it cannot go next to the serves_orange_county gate above. So it sits after
-- that select's "Job not found" guard AND after the cheap early-returns that
-- follow it (already assigned, not open, already applied): a bogus lead id
-- still gets the honest "Job not found" message, and a pro who already paid
-- for this lead and later narrowed launch_cities still gets the idempotent
-- `true` on a retry instead of a geography error. It runs well before ANY
-- money moves (get_or_create_wallet, the wallet FOR UPDATE, the bonus drain,
-- the wallet debit) or any row is written (the lead_applications insert, the
-- ledger row). Nothing between the select and the gate mutates anything.
--
