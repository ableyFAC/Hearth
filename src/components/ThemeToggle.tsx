...
export default function ThemeToggle({
  ...
  if (variant === "row") {
    return (
      <button
        ...
        role="switch"
        aria-checked={dark}
        className="mx-1 flex w-[calc(100%-0.5rem)] items-center justify-between gap-2 rounded-md px-3 py-2 text-left text-sm text-stone-700 hover:bg-hearth-50 dark:text-stone-300 dark:hover:bg-stone-600"
      >
        ...
        <span
          ...
          className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${
            dark ? "bg-hearth-600" : "bg-stone-300"
          }`}
        >
          <span
  ...
  return (
    <button
      ...
      className="focus-ring flex h-9 w-9 items-center justify-center rounded-full text-stone-500 hover:bg-hearth-50 hover:text-hearth-700 dark:text-stone-400 dark:hover:bg-stone-800 dark:hover:text-hearth-300"
    >
      {dark ? sun : moon}
    </button>
  );
